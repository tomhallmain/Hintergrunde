...
def get_log_directory():
    ...
    if system == 'windows':
        ...
        log_dir = Path(appdata) / 'Hintergrunde' / 'logs'
    else:
        # Use ~/.local/share for Linux/Mac
        log_dir = Path.home() / '.local' / 'share' / 'Hintergrunde' / 'logs'
...
def setup_logger(name, log_file='hintergrunde.log'):
    ...
    # Log the current log files
    log_files = sorted(glob.glob(str(log_dir / f"{log_file}*")))
    logger.info(f"Current log files: {log_files}")
