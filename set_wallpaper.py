...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage:")
        print("  Set specific wallpaper: python set_wallpaper.py <path_to_image>")
        print("  Rotate wallpaper: python set_wallpaper.py --rotate <path_to_image_directory> [--min-days 7]")
        sys.exit(1)
    ...
    if sys.argv[1] == '--rotate':
        ...
        if '--min-days' in sys.argv:
            try:
                min_days_index = sys.argv.index('--min-days')
                min_days = int(sys.argv[min_days_index + 1])
            except (ValueError, IndexError):
                print("Warning: Invalid --min-days value, using default of 7 days")
        
        rotate_wallpaper(sys.argv[2], min_days)
    else:
